...
fn main() -> Result<(), AutoGuiError> {
    let mut rustautogui = RustAutoGui::new(false)?; // initialize
    ...
    rustautogui.prepare_template_from_file(
    ...
    rustautogui.keyboard_command("win")?;

    thread::sleep(time::Duration::from_millis(500));

    // write in chrome
    rustautogui.keyboard_input("chrome")?;
    ...
    rustautogui.keyboard_multi_key("ctrl", "t", None)?;

    thread::sleep(time::Duration::from_millis(500));

    // input github in url (url input area selected automatically by new tab opening)
    rustautogui.keyboard_input("https://github.com/DavorMar/rustautogui")?;
    ...
    rustautogui.loop_find_stored_image_on_screen_and_move_mouse(0.95, 1.0, 15, "stars")?;
    ...
    rustautogui.loop_find_image_on_screen_and_move_mouse(0.95, 1.0, 15)?;
